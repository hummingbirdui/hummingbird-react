...
Built on the [Radix UI Slider](https://www.radix-ui.com/primitives/docs/components/slider) primitive. All props are forwarded to the Radix root; the track, range, and thumbs are rendered internally with Hummingbird's `slider-track`, `slider-range`, and `slider-thumb` classes.
