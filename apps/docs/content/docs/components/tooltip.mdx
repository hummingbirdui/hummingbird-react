...
Contains all the parts of a tooltip. Each `Tooltip` wraps itself in its own `TooltipProvider` with a `delayDuration` of `0`, so tooltips open instantly unless a delay is set. Renders no DOM element of its own.
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      description: (
        <>
          When <code>true</code>, trying to hover the content will result in
          the tooltip closing as the pointer leaves the trigger. Inherits from{" "}
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      description: (
        <>
          ...
          component. If this is not descriptive enough, or you have content
          that cannot be announced, use <code>aria-label</code> as a more
          descriptive label.
        </>
      ),
    },
    ...
    },
    {
      ...
      description: (
        <>
          The preferred side of the trigger to render against when open. Will
          be reversed when collisions occur and <code>avoidCollisions</code> is
          enabled.
        </>
      ),
    },
